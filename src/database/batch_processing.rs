...
/// collecting them into a FxHashMap. The key and value are extracted using provided
/// closures. Automatically uses parallel processing for large result sets (>= 1000 rows).
///
/// # Arguments
/// * `batches` - Slice of RecordBatch to process
...
///
/// # Returns
/// FxHashMap of extracted key-value pairs
    ...
    V: Send,
{
    ...
    if total_rows < BATCH_PARALLEL_THRESHOLD {
        ...
        let partial_maps: Vec<FxHashMap<K, V>> = batches
            .par_iter()
            .map(|batch| {
...
mod tests {
    ...
    fn test_build_map_empty() {
        ...
        let result: FxHashMap<i32, String> =
