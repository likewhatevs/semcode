...
impl DatabaseManager {
    ...
    async fn scan_table_for_duplicates(
        ...
        let results = table
            ...
            .try_collect::<Vec<_>>()
            .await?;
        let mut row_counts: FxHashMap<String, usize> = FxHashMap::default();
        let mut row_examples: FxHashMap<String, String> = FxHashMap::default();
    ...
    pub async fn insert_types(&self, types: Vec<TypeInfo>) -> Result<()> {
        ...
        let mut unique_definitions = FxHashSet::default();
        for type_info in &types {
            if !type_info.definition.is_empty() {
    ...
    pub async fn insert_macros(&self, macros: Vec<MacroInfo>) -> Result<()> {
        ...
        let mut unique_content = FxHashSet::default();
        for macro_info in &macros {
            if !macro_info.definition.is_empty() {
    ...
    pub async fn get_function_callers_git_aware(
        ...
        let valid_hashes: FxHashSet<String> = git_manifest.values().cloned().collect();
        tracing::info!(
    ...
    pub async fn get_entry_point_functions(&self) -> Result<Vec<String>> {
        ...
        let mut functions_with_calls = FxHashSet::default();
        let mut functions_that_are_called = FxHashSet::default();
    ...
    pub async fn get_functions_by_names(
        &self,
        names: &[String],
    ) -> Result<FxHashMap<String, FunctionInfo>> {
    ...
    pub async fn get_macros_by_names(
        &self,
        names: &[String],
    ) -> Result<FxHashMap<String, MacroInfo>> {
    ...
    pub async fn get_types_by_names(
        &self,
        names: &[String],
    ) -> Result<FxHashMap<String, TypeInfo>> {
    ...
    pub async fn get_typedefs_by_names(
        &self,
        names: &[String],
    ) -> Result<FxHashMap<String, TypedefInfo>> {
    ...
    pub async fn get_existing_git_file_shas(&self) -> Result<FxHashSet<String>> {
    ...
    pub async fn get_processed_file_pairs(&self) -> Result<FxHashSet<(String, String)>> {
    ...
    pub async fn get_existing_function_names(&self) -> Result<FxHashSet<String>> {
        use futures::TryStreamExt;
    ...
    pub async fn get_files_for_incremental_scan(
        ...
        let mut files_to_scan = FxHashSet::default();
    ...
    async fn find_files_connected_to_changes(
        ...
        let mut connected_files = FxHashSet::default();

        // Collect all file paths that changed
        let changed_file_paths: FxHashSet<String> =
    ...
    async fn find_files_connected_via_calls(
        ...
        let _changed_file_paths: FxHashSet<String> =
            changed_files.iter().map(|cf| cf.path.clone()).collect();
    ...
    async fn generate_git_manifest(&self, git_sha: &str) -> Result<FxHashMap<String, String>> {
    ...
    async fn function_exists_in_manifest(
        &self,
        function_name: &str,
        git_manifest: &FxHashMap<String, String>,
    ) -> Result<bool> {
