...
impl ContentStore {
    ...
    /// Determine which shard an xxHash3 hash belongs to based on its first hex character
    fn get_shard_number(xxhash: &str) -> u8 {
        if xxhash.is_empty() {
            0
        } else {
    ...
    /// Get the table name for an xxHash3 hash
    fn get_table_name_for_hash(xxhash: &str) -> String {
        let shard = Self::get_shard_number(xxhash);
        Self::get_shard_table_name(shard)
    }

    /// Insert content and return the xxHash3 hash - uses upsert
    pub async fn store_content(&self, content: &str) -> Result<String> {
        let xxhash = hash::compute_xxhash(content);
    ...
    pub async fn insert_batch(&self, content_items: Vec<ContentInfo>) -> Result<()> {
        ...
        // Deduplicate content items by xxhash within the batch
        let mut dedup_map: FxHashMap<String, ContentInfo> = FxHashMap::default();
        for item in content_items {
            dedup_map.insert(item.xxhash.clone(), item);
        }
        ...
        for item in deduplicated_items {
            let shard = Self::get_shard_number(&item.xxhash);
    ...
    async fn insert_chunk(
        ...
        // Create xxhash StringArray (non-nullable)
        let mut xxhash_builder = StringBuilder::new();
        for item in content_items {
            xxhash_builder.append_value(&item.xxhash);
        }
        let xxhash_array = xxhash_builder.finish();

        let schema = self.get_schema();

        let batch = RecordBatch::try_from_iter(vec![
            ("xxhash", Arc::new(xxhash_array) as ArrayRef),
        ...
        let mut merge_insert = table.merge_insert(&["xxhash"]);
        merge_insert
    ...
    pub async fn content_exists(&self, xxhash: &str) -> Result<bool> {
        let table_name = Self::get_table_name_for_hash(xxhash);
        let table = self.connection.open_table(&table_name).execute().await?;

        // Use indexed lookup with hex string
        let results = table
            .query()
            .only_if(format!("xxhash = '{xxhash}'"))
            .limit(1)
            .execute()
            .await?
            .try_collect::<Vec<_>>()
    ...
    /// Get content by xxHash3 hash - uses indexed lookup
    ...
    /// Get content by xxHash3 hash hex string (now redundant since get_content takes hex strings directly)
    pub async fn get_content_by_hex(&self, xxhash_hex: &str) -> Result<Option<String>> {
        self.get_content(xxhash_hex).await
    }
    ...
    pub async fn get_content_bulk(&self, hashes: &[String]) -> Result<FxHashMap<String, String>> {
        ...
        for (shard, shard_hashes) in shard_groups {
            ...
            for chunk in shard_hashes.chunks(chunk_size) {
                ...
                let filter = format!("xxhash IN ({in_clause})");
                ...
                for batch in &results {
                    ...
                    for i in 0..batch.num_rows() {
                        let hash = xxhash_array.value(i).to_string();
    ...
    fn extract_content_from_batch(
        ...
        Ok(Some(ContentInfo {
            xxhash: xxhash_array.value(row).to_string(),
    ...
    fn get_schema(&self) -> Arc<Schema> {
        Arc::new(Schema::new(vec![
            Field::new("xxhash", DataType::Utf8, false), // xxHash3 hash as hex string
...
pub struct ContentInfoJson {
    pub xxhash: String, // Hex string for JSON
    pub content: String,
}
...
impl From<ContentInfo> for ContentInfoJson {
    fn from(content_info: ContentInfo) -> Self {
        Self {
            xxhash: content_info.xxhash, // Already a hex string
