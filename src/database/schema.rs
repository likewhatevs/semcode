...
impl SchemaManager {
    ...
    async fn create_content_table(&self) -> Result<()> {
        let schema = Arc::new(Schema::new(vec![
            Field::new("xxhash", DataType::Utf8, false), // xxHash3 hash of content as hex string
    ...
    pub async fn create_scalar_indices(&self) -> Result<()> {
        ...
        for shard in 0..16u8 {
            ...
            if table_names.iter().any(|n| n == &table_name) {
                ...
                // Primary index on xxhash for deduplication and fast lookups
                self.try_create_index(
                    &table,
                    &["xxhash"],
                    &format!("BTree index on {table_name}.xxhash"),
                )
                .await;
