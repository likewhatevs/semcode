...
impl TypeStore {
    ...
    async fn insert_chunk(&self, table: &lancedb::table::Table, types: &[TypeInfo]) -> Result<()> {
        ...
        let _definition_hash_strings: Vec<Option<String>> = types
            ...
            .map(|type_info| {
                if type_info.definition.is_empty() {
                    ...
                    Some(crate::hash::compute_xxhash(&type_info.definition))
                }
            })
            .collect();
        ...
        for item in types {
            if item.definition.is_empty() {
                ...
                definition_hash_builder.append_value(crate::hash::compute_xxhash(&item.definition));
            }
        }
                ...
                for i in 0..batch.num_rows() {
                    if let Ok(Some(type_data)) = self.extract_type_metadata_from_batch(batch, i) {
                        ...
                        let definition = match type_data.definition_hash {
                            Some(ref hash) => format!("[xxhash:{}]", hex::encode(hash)),
                            None => String::new(),
                        };
    ...
    pub async fn get_all_bulk_optimized(&self) -> Result<Vec<TypeInfo>> {
        ...
        let mut all_definition_hashes = FxHashSet::default();
        let batch_size = 10000;
        let mut offset = 0;
    ...
    async fn bulk_get_content(&self, hashes: &[String]) -> Result<FxHashMap<String, String>> {
    ...
    pub async fn get_by_names(&self, names: &[String]) -> Result<FxHashMap<String, TypeInfo>> {
        if names.is_empty() {
    ...
    fn get_schema(&self) -> Arc<Schema> {
        Arc::new(Schema::new(vec![
            ...
            Field::new("definition_hash", DataType::Utf8, true), // xxHash3 hash referencing content table as hex string (nullable for empty definitions)
...
impl TypedefStore {
    ...
    pub async fn get_by_names(&self, names: &[String]) -> Result<FxHashMap<String, TypedefInfo>> {
        if names.is_empty() {
            return Ok(FxHashMap::default());
        }
...
impl MacroStore {
        ...
        let _definition_hash_strings: Vec<Option<String>> = macros
            ...
            .map(|macro_info| {
                if macro_info.definition.is_empty() {
                    ...
                    Some(crate::hash::compute_xxhash(&macro_info.definition))
                }
            })
            .collect();
    ...
    async fn insert_metadata_chunk(
        ...
        let _definition_hash_strings: Vec<String> = macros
            ...
            .map(|macro_info| {
                if !macro_info.definition.is_empty() {
                    crate::hash::compute_xxhash(&macro_info.definition)
                } else {
    ...
    pub async fn get_by_names(&self, names: &[String]) -> Result<FxHashMap<String, MacroInfo>> {
        if names.is_empty() {
            return Ok(FxHashMap::default());
        }
